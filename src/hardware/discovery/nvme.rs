...
use super::{NvmeNamespaceInfo, SmartData};
...
impl NvmeDiscovery {
    ...
    pub async fn get_controller_info(device: &str) -> Result<NvmeControllerInfo> {
        ...
        let json: serde_json::Value =
            serde_json::from_slice(&output.stdout).map_err(|e| Error::NvmeCommand {
                command: "id-ctrl".into(),
    ...
    pub async fn get_smart_data(device: &str) -> Result<SmartData> {
        ...
        Ok(SmartData {
            temperature_celsius: (json["temperature"].as_i64().unwrap_or(273) - 273) as i32,
...
mod tests {
    ...
    fn test_extract_controller_path() {
        ...
        assert_eq!(
            NvmeDiscovery::extract_controller_path("/dev/nvme10n1").unwrap(),
            "/dev/nvme10"
        );
    }
    ...
    #[test]
    fn test_nvme_features_default() {
        let features = NvmeFeatures::default();
        assert!(!features.zns);
        assert!(!features.ns_management);
        assert!(!features.multipath);
    }
}
