...
use super::{DriveInfo, DriveType, NodeHardwareInfo, NvmeNamespaceInfo};
...
impl HardwareScanner {
    ...
    async fn scan_nvme_namespace(&self, sysfs_path: &Path) -> Result<DriveInfo> {
        ...
        let firmware = self
            .read_sysfs_attr(sysfs_path, "device/firmware_rev")
        ...
        let sectors: u64 = size_str
            .trim()
            .parse()
            .map_err(|_| Error::HardwareDiscovery(format!("Invalid size: {}", size_str)))?;
        let capacity_bytes = sectors * 512;

        // Read block size
        let block_size_str = self
            .read_sysfs_attr(sysfs_path, "queue/logical_block_size")
    ...
    async fn scan_block_device(&self, sysfs_path: &Path) -> Result<DriveInfo> {
        ...
        // Read device info
        let model = self
            .read_sysfs_attr(sysfs_path, "device/model")
            .or_else(|_| self.read_sysfs_attr(sysfs_path, "device/name"))
            .unwrap_or_else(|_| "Unknown".to_string());

        let serial = self
            .read_sysfs_attr(sysfs_path, "device/serial")
            .unwrap_or_else(|_| "Unknown".to_string());

        let firmware = self
            .read_sysfs_attr(sysfs_path, "device/firmware")
    ...
    fn read_sysfs_attr(&self, base_path: &Path, attr: &str) -> Result<String> {
        ...
        fs::read_to_string(&path).map_err(|e| {
            Error::HardwareDiscovery(format!("Failed to read {}: {}", path.display(), e))
        })
    }
...
mod tests {
    ...
    fn test_scanner_default() {
        let scanner = HardwareScanner::default();
        assert_eq!(scanner.config().min_size_bytes, 1_000_000_000);
    }
}
