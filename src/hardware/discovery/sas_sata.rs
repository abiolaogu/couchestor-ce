...
impl SasSataDiscovery {
    ...
    pub async fn get_device_info(device: &str) -> Result<SataDeviceInfo> {
        let output = Command::new("smartctl")
            ...
            .output()
            .map_err(|e| {
                Error::HardwareDiscovery(format!("smartctl failed for {}: {}", device, e))
            })?;

        // smartctl returns non-zero for various warnings, check output
        let json: serde_json::Value = serde_json::from_slice(&output.stdout).map_err(|e| {
            Error::HardwareDiscovery(format!("smartctl JSON parse error for {}: {}", device, e))
        })?;
        ...
        let capacity_bytes = json["user_capacity"]["bytes"].as_u64().unwrap_or(0);

        let logical_sector_size = json["logical_block_size"].as_u64().unwrap_or(512) as u32;

        let physical_sector_size = json["physical_block_size"].as_u64().unwrap_or(512) as u32;

        let rotation_rate = json["rotation_rate"].as_u64().unwrap_or(0) as u32;
    ...
    pub async fn get_smart_data(device: &str) -> Result<SmartData> {
        let output = Command::new("smartctl")
            .args(["-A", "-H", "-j", device])
            .output()
            .map_err(|_| Error::SmartUnavailable {
                device: device.to_string(),
            })?;

        let json: serde_json::Value =
            serde_json::from_slice(&output.stdout).map_err(|_| Error::SmartUnavailable {
                device: device.to_string(),
            })?;
        ...
        // Power on hours
        let power_on_hours = Self::find_smart_attr(attrs, &[9]).unwrap_or(0);

        // Data read/written (may not be available for all drives)
        let data_units_read = Self::find_smart_attr(attrs, &[241]).unwrap_or(0);
        let data_units_written = Self::find_smart_attr(attrs, &[242]).unwrap_or(0);
    ...
    pub async fn run_self_test(device: &str, test_type: &str) -> Result<()> {
        ...
        let output = Command::new("smartctl")
            ...
            .output()
            .map_err(|e| {
                Error::HardwareDiscovery(format!("Failed to start SMART test on {}: {}", device, e))
            })?;
    ...
    pub async fn list_devices() -> Result<Vec<String>> {
        let output = Command::new("smartctl")
            .args(["--scan", "-j"])
            .output()
            .map_err(|e| Error::HardwareDiscovery(format!("smartctl scan failed: {}", e)))?;
...
mod tests {
    ...
    fn test_find_smart_attr() {
        ...
        assert_eq!(
            SasSataDiscovery::find_smart_attr(&attrs, &[9]),
            Some(12345)
        );
        assert_eq!(SasSataDiscovery::find_smart_attr(&attrs, &[999]), None);
        assert_eq!(
            SasSataDiscovery::find_smart_attr(&attrs, &[190, 194]),
            Some(42)
        );
    }

    #[test]
    fn test_sata_device_info() {
        let info = SataDeviceInfo {
        ...
        assert!(info.is_ssd);
        assert_eq!(info.rotation_rate, 0);
    }
}
